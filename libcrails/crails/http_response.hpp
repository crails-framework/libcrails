#ifndef  HTTP_RESPONSE
# define HTTP_RESPONSE

# include "server/connection.hpp"
# include <crails/render_target.hpp>
# include <atomic>
# include <unordered_map>
# include <string_view>

namespace Crails
{
  class BuildingResponse : public RenderTarget
  {
  public:
    BuildingResponse(Connection& connection) : connection(connection)
    {}

    void set_response(HttpStatus code, std::string&&);
    void set_response(HttpStatus code, const std::string& body) { set_response(code, std::string_view(body)); }
    void set_response(HttpStatus code, const std::string_view body);
    void set_status_code(HttpStatus code) { get_raw_response().result(code); }
    void set_header(boost::beast::http::field key, const std::string& value) { if (!sent()) get_raw_response().set(key, value); }
    void set_header(const std::string& key, const std::string& value) override { if (!sent()) get_raw_response().set(key, value); }
    void set_body(const char* str, size_t size) override;
    void set_body(std::string&& body) override;
    void send();
    bool sent() const { return already_sent.test(); }

    HttpStatus get_status_code() const { return get_raw_response().result(); }
    HttpResponse& get_raw_response() { return connection.get_response(); }
    const HttpResponse& get_raw_response() const { return connection.get_response(); }

  private:
    Connection& connection;
    std::atomic_flag already_sent = ATOMIC_FLAG_INIT;
  };
}

#endif
