...
void Connection::on_body_too_large()
{
  response.result(HttpStatus::payload_too_large);
  response.set(HttpHeader::content_length, "0");
...
void Connection::on_read_error(boost::beast::error_code ec)
{
  ...
  else if (ec == boost::beast::http::error::body_limit)
    on_body_too_large();
  else
